...
/*-----------------------SHUVO AHMED---------------------------*/ mtc inline void solve()
{
    ...
    For(i, n) vp.push_back({vec[i], vec2[i]});
    Sort(vp);
    vector<pair<ll, ll>> vp_ans;
    For(i, n)
    {
        if (vec[i] == vp[i].ff && vec2[i] == vp[i].ss)
        {
        }
        else
        {
            for (ll j = i + 1; j < n; j++)
            {
                if (vec[j] == vp[i].ff && vec2[j] == vp[i].ss)
                {
                    swap(vec[i], vec[j]);
                    swap(vec2[i], vec2[j]);
                    vp_ans.push_back({i + 1, j + 1});
                }
            }
        }
    }
    ...
    for (ll i = 1; i < n; i++)
    {
        if (vp[i].ff >= vp[i - 1].ff && vp[i].ss >= vp[i - 1].ss)
        {
        }
        else
            test = false;
