        ...
        for (ll j = 0; j < len; j++)
        {
            if (i != j)
            {
                if (vec[i][0] == vec[j][0])
                {
                    ...
                    if (cunt == vec[i].size() && vec[j].size() > vec[i].size())
                    {
                        for (ll p = 0; p < len; p++)
                        {
                            if (vec[p][0] == vec[j][cunt])
                            {
                                for (ll d = cunt; d < vec[j].size(); d++)
                                {
                                    if (vec[j][d] == vec[p][d - cunt])
                                        c++;
                                    else
                                    {
                                        c = 0;
                                ...
                                if (c == vec[p].size() && vec[p].size() + cunt == vec[j].size())
                                    cout << vec[j] << nl;
                            }
                        }
                    }
